...
export default function Redemption() {
  ...
  const redeem = async () => {
    ...
    try {
      const r = await points.redeem({ customerWallet: wa, amount: amt.toString(), tokenContract: "0x0000000000000000000000000000000000000001" });
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8">
      <Card>
        <CardHeader className="text-center"><CardTitle>Redeem Points</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button className="w-full" onClick={redeem} disabled={loading}>{loading ? "..." : "Redeem"}</Button>
        </CardContent>
      </Card>
    </div>
  );
